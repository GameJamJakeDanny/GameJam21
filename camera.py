...
# im not really sure exactly whats going on here tbh, but it works?
class ScrollManager:
    ...
    def set_default_view(self):
        self.views["right"] = self.window.player.center_x + (self.width/2)
        self.views["left"] = self.window.player.center_x - (self.width/2)
        self.views["top"] = self.window.player.center_y + (self.height/2)
        self.views["bottom"] = self.window.player.center_y - (self.height/2)
        self.update_views()
    ...
    def check_views(self):
        ...
        if limits['top'] is not None and view['top'] > limits['top']:
